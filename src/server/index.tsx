...
app.get('*',async c=>{
  ...
  const title=initial.profile?`${initial.profile.name} — glutenfree.mt`:initial.detail?`${initial.detail.place.name} — glutenfree.mt`:'glutenfree.mt — Good food. Shared knowledge.';
  ...
  return c.html(html,(path.startsWith('/places/')&&!initial.detail)||(path.startsWith('/users/')&&!initial.profile)?404:200);
});
app.onError((err,c)=>{
