...
Conditions allow multiple optional choices: Coeliac, Gluten intolerant, Gluten sensitive, Wheat allergy, and Other. Symptom frequency after eating gluten or wheat is optional; an unanswered field is distinct from “I don’t know.” These self-reported answers are private by default. One explicit sharing switch makes them visible on `/users/:id`; disabling it hides them without clearing the private answers. Public APIs, contribution identities, and rendered page data omit unshared answers. Email and permissions never appear on public profiles.

Avatar originals accept JPEG, PNG, or WebP up to 10 MB. The browser crops and strips metadata, then sends a JPEG no larger than 256 × 256 pixels and 100,000 bytes. The server validates and re-encodes it independently. Only one current image is stored per user in the existing R2 bucket under `avatars/`; presets store no image objects. Replacements and removals delete old objects immediately where possible. A durable cleanup queue retries failed removals and account-deletion cleanup after 24 hours, in batches of 100 per daily run. Uploads are queued before storage so interrupted uploads are also recoverable. Avatar responses are not cached by the browser or service worker.

Migration `0007_user_profiles.sql` extends profiles without changing roles and adds avatar cleanup triggers. It is applied locally in this workspace. Apply pending migrations before deploying to other environments. No additional service or dependency is required. Tests in `tests/profiles.test.ts` and `tests/profiles.e2e.ts` cover authorization, privacy, current author identities, image processing, cleanup, and mobile editing. Live provider login and Cloudflare R2 operations still need staging validation.
